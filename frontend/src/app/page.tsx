...
import { AlertTriangle, ArrowUpDown, CheckCircle2, CircleStop, FileAudio, FileText, HeartPulse, Languages, Leaf, LockKeyhole, LogOut, Mic, Printer, RefreshCw, Search, ShieldAlert, Stethoscope, Trash2, Upload, UserRound, Volume2 } from "lucide-react";
...
export default function AyushOPDApp() {
  ...
  const nextQuestion = async () => {
    ...
    try {
      const response = await axios.post<{ follow_up?: string }>(`${API_URL}/audio/follow-up`, { question: QUESTIONS[language][questionIndex], answer: answers[questionIndex], language: LANGUAGE_OPTIONS.find((option) => option.code === language)?.label ?? "English" });
  ...
  const startFollowUpVoice = () => {
    const speechConstructor = (window as Window & { SpeechRecognition?: SpeechRecognitionConstructor; webkitSpeechRecognition?: SpeechRecognitionConstructor }).SpeechRecognition
      ?? (window as Window & { webkitSpeechRecognition?: SpeechRecognitionConstructor }).webkitSpeechRecognition;
    if (!speechConstructor) { setStatus("Voice input is not supported in this browser. You can type your clarification instead."); return; }
    const recognition = new speechConstructor();
    recognition.continuous = false; recognition.interimResults = true; recognition.lang = getSpeechCode(language);
    setFollowUpListening(true);
    recognition.onresult = (event) => {
      let transcript = "";
      for (let index = event.resultIndex; index < event.results.length; index += 1) transcript += event.results[index][0].transcript;
      setFollowUpAnswer(transcript.trim());
      setFollowUpListening(false);
    };
    recognition.onerror = () => { setFollowUpListening(false); setStatus("Voice input stopped. You can type your clarification instead."); };
    recognition.start();
  };
  ...
  return <main className="app-shell">
    ...
    <header className="topbar"><div className="brand"><Image src="/nivara-logo.svg" alt="NIVARA - For a Healthier Tomorrow" width={285} height={102} priority className="brand-logo" /></div><div className="government-lockup"><Image src="/ayush-logo.svg" alt="Ministry of AYUSH" width={290} height={58} className="ayush-logo" /></div><div className="topbar-meta"><span className="live-dot" /> Secure clinical intake</div></header>
...
function PatientPortal(props: PatientProps) {
  ...
  const answeredCount = props.answers.filter((answer) => answer.trim()).length;
  const completion = Math.round((answeredCount / QUESTIONS[props.language].length) * 100);
...
function DoctorPortal({ cases, refresh, deleteCase }: { cases: CaseRecord[]; refresh: () => void; deleteCase: (caseId: number, patientName: string) => void }) {
  ...
  const redFlagCount = cases.filter((record) => record.red_flags && record.red_flags !== "None identified").length;
  return <div className="doctor-view"><div className="doctor-header"><div><p className="eyebrow">DOCTOR PORTAL / CLINICAL QUEUE</p><h1>Patient briefs</h1><p>Structured intake summaries, ready for review.</p></div><button className="refresh-button" onClick={refresh}><RefreshCw size={16} /> Refresh queue</button></div><div className="doctor-stats"><div><span className="stat-icon"><UserRound size={16} /></span><strong>{cases.length}</strong><span>Total briefs</span></div><div><span className="stat-icon attention"><ShieldAlert size={16} /></span><strong>{redFlagCount}</strong><span>Needs review</span></div><div><span className="stat-icon ready"><CheckCircle2 size={16} /></span><strong>{cases.length - redFlagCount}</strong><span>Ready to review</span></div></div><div className="queue-toolbar"><label className="queue-search"><Search size={16} /><span className="sr-only">Search patient briefs</span><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search patients or symptoms" /></label><button className={redFlagsOnly ? "filter-button active" : "filter-button"} onClick={() => setRedFlagsOnly((value) => !value)}><ShieldAlert size={15} /> Red flags only</button><button className="filter-button" onClick={() => setSortOrder((value) => value === "newest" ? "oldest" : "newest")}><ArrowUpDown size={15} /> {sortOrder === "newest" ? "Newest first" : "Oldest first"}</button></div><div className="queue-meta"><span>{visibleCases.length} of {cases.length} briefs shown</span><span>{redFlagCount} need review</span></div>{cases.length === 0 ? <div className="empty-state"><Stethoscope size={32} /><h2>No patient briefs yet</h2><p>Completed patient intakes will appear here.</p></div> : visibleCases.length === 0 ? <div className="empty-state"><Search size={32} /><h2>No matching briefs</h2><p>Try another patient name, symptom, or filter.</p></div> : <div className="case-list">{visibleCases.map((record) => {
